...
const earningsUtils_1 = require("../utils/earningsUtils");
...
async function awardPoints(userId, points, description, pendingUntil, expiresAt) {
    ...
    if (!isPending) {
        ...
        if (user) {
            ...
            (0, socket_1.emitToUser)(userId, "points:update", { points: user.points });
        }
    }
...
router.post("/", auth_1.requireAuth, (0, validate_1.validate)(order_validators_1.createOrderSchema), async (req, res) => {
    try {
        const { fuelId, fuelTypeId, stationId, quantity, deliveryAddressId, cylinderType, deliveryType, cylinderImages, returnSwapAt, note, } = req.body;
        ...
        const fuelLookup = fuelId
            ? FuelType_1.default.findById(fuelId)
            : (() => {
                const slug = (fuelTypeId ?? "").toLowerCase();
                const matcher = SLUG_TO_NAME[slug];
                return matcher
                    ? FuelType_1.default.findOne({ name: matcher })
                    : Promise.resolve(null);
            })();
        const [fuel, station, address] = await Promise.all([
            fuelLookup,
            Station_1.default.findById(stationId),
            Address_1.default.findById(deliveryAddressId),
        ]);
        if (!fuel)
        ...
        if (!station.isActive)
            return res.status(400).json({ message: "Station is currently closed" });
        if (!address)
            return res.status(404).json({ message: "Delivery address not found" });
        // Use station's per-fuel price (vendor-set), falling back to FuelType global price
        const resolvedFuelId = fuel._id.toString();
        const stationFuelEntry = station.fuels.find((f) => f.fuel.toString() === resolvedFuelId);
        if (!stationFuelEntry) {
        ...
        if (stationHasCoords && addressHasCoords) {
            const distanceKm = (0, haversine_1.haversineDistance)({ lat: station.location.lat, lng: station.location.lng }, { lat: address.latitude, lng: address.longitude });
            deliveryFee = (0, haversine_1.calcDeliveryFee)(distanceKm);
        }
        else {
        ...
        let order = await Order_1.default.create(orderData);
        ...
        if (station.autoAcceptOrders) {
            ...
            (0, socket_1.emitToUser)(req.userId, "order:update", { orderId: String(order._id), status: "confirmed" });
            if (station.vendorId) {
                const vendorId = station.vendorId.toString();
                (0, socket_1.emitToUser)(vendorId, "order:new", { orderId: String(order._id), status: "confirmed", fuelName: fuel.name, quantity, unit: fuel.unit });
                // Create vendor earnings + emit earnings event (fast DB write)
                (0, earningsUtils_1.createVendorPendingEarning)(vendorId, order._id.toString(), fuelCost).catch(() => { });
            }
            // 2. Background: DB notifications + push (do not block response)
            Promise.all([
                (0, notify_1.notifyUser)(req.userId, "order", "Order Confirmed", `Your ${fuel.name} order (${quantity} ${fuel.unit}) was automatically confirmed. A rider will be assigned shortly.`),
                station.vendorId
                    ? (0, notify_1.notifyUser)(station.vendorId.toString(), "order", "Order Auto-Confirmed", `A ${fuel.name} order (${quantity} ${fuel.unit}) was auto-confirmed at your station.`)
                    : Promise.resolve(),
                station.vendorId
                    ? (0, notify_1.notifyUser)(station.vendorId.toString(), "payment", "Earnings Added", `₦${fuelCost.toLocaleString()} added to your pending earnings.`)
                    : Promise.resolve(),
            ]).catch(() => { });
        }
        else {
            ...
            if (station.vendorId) {
                const vendorId = station.vendorId.toString();
                (0, socket_1.emitToUser)(vendorId, "order:new", { orderId: String(order._id), status: "pending", fuelName: fuel.name, quantity, unit: fuel.unit });
                (0, notify_1.notifyUser)(vendorId, "new_order", "New Order Received", `A new ${fuel.name} order (${quantity} ${fuel.unit}) has been placed at your station.`).catch(() => { });
            }
            (0, notify_1.notifyUser)(req.userId, "order", "Order Placed", `Your ${fuel.name} order (${quantity} ${fuel.unit}) has been placed successfully.`).catch(() => { });
        }
...
router.get("/:orderId", auth_1.requireAuth, async (req, res) => {
    try {
        ...
        if (order.riderId) {
            const RiderProfile = (await Promise.resolve().then(() => __importStar(require("../models/RiderProfile")))).default;
            const rp = await RiderProfile.findOne({
                user: typeof order.riderId === "object" &&
                    order.riderId !== null &&
...
router.get("/:orderId/route", auth_1.requireAuth, async (req, res) => {
    try {
        const order = await Order_1.default.findById(req.params.orderId)
         ...
         */
        const target = req.query.target === "station" ? "station" : "destination";
        let destLat;
        let destLng;
        if (target === "station") {
        ...
        const cacheKey = `${order._id.toString()}:${target}:${riderLat.toFixed(3)},${riderLng.toFixed(3)}`;
        ...
        const directionsRes = await fetch(`https://maps.googleapis.com/maps/api/directions/json?${params}`);
        ...
        // back synchronously).
        try {
            const { emitToDelivery } = await Promise.resolve().then(() => __importStar(require("../socket")));
            const Delivery = (await Promise.resolve().then(() => __importStar(require("../models/Delivery")))).default;
...
const transitions_1 = require("../_shared/transitions");
router.patch("/:orderId/status", auth_1.requireAuth, (0, validate_1.validate)(order_validators_1.updateOrderStatusSchema), async (req, res) => {
    try {
        ...
        const caller = await User_1.default.findById(req.userId).select("role").lean();
        if (!caller)
        ...
        if (role === "vendor") {
            const GasStation = (await Promise.resolve().then(() => __importStar(require("../models/Station")))).default;
            const station = await GasStation.findById(order.station)
                .select("vendorId")
                .lean();
            if (!station || station.vendorId?.toString() !== req.userId) {
                return res.status(403).json({ message: "Forbidden" });
            }
        }
        if (!(0, transitions_1.isAllowedTransition)(role, prevStatus, status)) {
        ...
        (0, socket_1.emitToUser)(userId, "order:update", { orderId: String(order._id), status });
        // Background notifications (do not block response)
        Promise.resolve().then(async () => {
            if (status === "confirmed" && prevStatus !== "confirmed") {
                await (0, notify_1.notifyUser)(userId, "order", "Order Confirmed", "Your order has been confirmed by the station.");
            }
            if ((status === "in_transit" || status === "in-transit") && prevStatus !== status) {
                await (0, notify_1.notifyUser)(userId, "delivery", "On the Way!", "Your fuel is on its way. Track your rider in real time.");
            }
            ...
            if (status === "arrived" && prevStatus !== "arrived") {
                await (0, notify_1.notifyUser)(userId, "delivery", "Your rider is here 🚪", "They're at your gate. Step out when ready.");
            }
            if (status === "delivered" && prevStatus !== "delivered") {
                const expiresAt = new Date();
                expiresAt.setDate(expiresAt.getDate() + 30);
                await awardPoints(userId, POINTS_CONFIG.orderDelivered, "Points for order delivered", undefined, expiresAt);
                await (0, earningsUtils_1.settleOrderEarnings)(order._id.toString());
                await (0, notify_1.notifyUser)(userId, "delivered", "Fuel Delivered!", "Your fuel order has been delivered. Enjoy!");
                await (0, notify_1.notifyUser)(userId, "points", "Points Earned!", `You earned ${POINTS_CONFIG.orderDelivered} Gaznger Points for this delivery.`);
            }
        }).catch(() => { });
        res.json(order);
    }
...
router.patch("/:orderId/cancel", auth_1.requireAuth, async (req, res) => {
    try {
        ...
        if (!order) {
            ...
            // rare error path.
            const exists = await Order_1.default.findById(req.params.orderId)
                .select("user status")
                .lean();
            if (!exists)
                return res.status(404).json({ message: "Order not found" });
        ...
        (0, socket_1.emitToUser)(req.userId, "order:update", { orderId: String(order._id), status: "cancelled" });
        await (0, notify_1.notifyUser)(req.userId, "cancelled", "Order Cancelled", "Your order has been cancelled and any earned points have been reversed.");
 ...
 */
router.post("/:orderId/customer-here", auth_1.requireAuth, async (req, res) => {
    try {
        const order = await Order_1.default.findById(req.params.orderId);
        if (!order)
            return res.status(404).json({ message: "Order not found" });
        ...
        if (order.riderId) {
            (0, socket_1.emitToUser)(order.riderId.toString(), "order:customer-here", {
                orderId: order._id.toString(),
                customerHereAt,
            });
            // Push to the rider device — non-fatal if it fails.
            (0, notify_1.notifyUser)(order.riderId.toString(), "delivery", "Customer at the gate", "Heads up — they're outside waiting.").catch(() => { });
        }
...
router.patch("/:orderId/confirm-delivery", auth_1.requireAuth, async (req, res) => {
    try {
        ...
        // null back and a 409.
        const existing = await Order_1.default.findById(req.params.orderId)
        ...
        const fallbackTotalCharged = existing.totalCharged ?? existing.totalPrice;
        const order = await Order_1.default.findOneAndUpdate({
            _id: req.params.orderId,
            user: req.userId,
        ...
        const completedDelivery = await Delivery_1.default.findOneAndUpdate({ order: order._id, status: "awaiting_confirmation" }, { status: "delivered", customerConfirmedAt: new Date() }, { new: true });
        ...
        if (completedDelivery) {
            const { leaveDeliveryRoom } = await Promise.resolve().then(() => __importStar(require("../socket")));
            leaveDeliveryRoom(String(completedDelivery._id));
        }
        // Increment rider's totalDeliveries
        if (order.riderId) {
            const RiderProfile = (await Promise.resolve().then(() => __importStar(require("../models/RiderProfile")))).default;
            await RiderProfile.findOneAndUpdate({ user: order.riderId }, { $inc: { totalDeliveries: 1 } });
        }
        ...
        // a follow-up fetch.
        (0, socket_1.emitToUser)(userId, "order:update", {
            orderId: String(order._id),
            status: "delivered",
        ...
        if (order.riderId) {
            (0, socket_1.emitToUser)(order.riderId.toString(), "order:update", { orderId: String(order._id), status: "delivered" });
        }
        Promise.all([
            (0, notify_1.notifyUser)(userId, "delivered", "Fuel Delivered!", "Your fuel order has been confirmed as delivered. Enjoy!"),
            (0, notify_1.notifyUser)(userId, "points", "Points Earned!", `You earned ${POINTS_CONFIG.orderDelivered} Gaznger Points for completing your delivery!`),
            order.riderId
                ? (0, notify_1.notifyUser)(order.riderId.toString(), "payment", "Earnings Settled!", "Your delivery earnings have been settled.")
                : Promise.resolve(),
        ]).catch(() => { });
...
// ===================== RATE A STATION =====================
router.post("/:orderId/rate", auth_1.requireAuth, (0, validate_1.validate)(order_validators_1.rateOrderSchema), async (req, res) => {
    try {
        ...
        const existingRating = await Rating_1.default.findOne({ order: order._id });
        if (existingRating)
        ...
        if (tipAmount > 0 && order.riderId) {
            try {
                const { getOrCreateUserWallet, postTransfer } = await Promise.resolve().then(() => __importStar(require("../utils/wallet")));
                ...
                if (customerWallet.available >= tipAmount) {
                    ...
                    (0, socket_1.emitToUser)(req.userId, "wallet:update", {
                        available: fresh.available,
                        pending: fresh.pending,
                    });
                    const freshRider = await getOrCreateUserWallet(order.riderId);
                    (0, socket_1.emitToUser)(order.riderId.toString(), "wallet:update", {
                        available: freshRider.available,
                        pending: freshRider.pending,
                    });
                    await (0, notify_1.notifyUser)(order.riderId.toString(), "payment", "Tip received", `Customer tipped you ₦${tipAmount.toLocaleString()} for order #${order._id
                        .toString()
                        .slice(-6)
                        .toUpperCase()}.`);
                }
 ...
 */
router.get("/:orderId/receipt.pdf", auth_1.requireAuth, async (req, res) => {
    try {
        const order = await Order_1.default.findById(req.params.orderId)
        ...
        const customer = await User_1.default.findById(req.userId)
            .select("displayName email")
            .lean();
        const fuel = order.fuel;
        const station = order.station;
        const addr = order.deliveryAddress;
        const orderShortId = order._id.toString().slice(-6).toUpperCase();
        const fuelLine = `${order.quantity} ${order.unit ?? fuel?.unit ?? "L"} ${fuel?.name ?? ""}`.trim();
        // paymentMethodLabel mirrors the mobile helper so the receipt
        // says the same thing the in-app receipt did.
        const paymentMethodLabel = labelForPaymentMethod(order.paymentMethodId, customer?.lastPaystackAuth);
        const totalCharged = order.totalCharged ?? order.totalPrice ?? 0;
        const { streamReceiptPdf } = await Promise.resolve().then(() => __importStar(require("../utils/receiptPdf")));
        res.setHeader("Content-Type", "application/pdf");
        res.setHeader("Content-Disposition", `inline; filename="gaznger-receipt-${orderShortId}.pdf"`);
...
function labelForPaymentMethod(paymentMethodId, lastPaystackAuth) {
    if (!paymentMethodId)
        return "Card";
    ...
    if (paymentMethodId === "card-new")
        return "Card";
    if (paymentMethodId === "wallet")
        return "Gaznger wallet";
    if (paymentMethodId === "transfer")
