...
router.get("/", async (req, res) => {
    try {
        const { verified, state, lga, lat, lng, radius, search, } = req.query;
        const filter = {};
...
router.post("/", auth_1.requireAuth, upload.single("image"), async (req, res) => {
    try {
        ...
        const imageUpload = await new Promise((resolve, reject) => {
            const stream = cloudinary_1.default.uploader.upload_stream({ resource_type: "image", folder: "stations" }, (error, result) => {
                if (error)
                    reject(error);
                else
                    resolve(result);
        ...
        const station = await Station_1.default.create({
            name,
            address,
            state,
            lga,
    ...
    try {
        ...
        if (req.file) {
            if (station.image) {
                ...
                if (publicId)
                    await cloudinary_1.default.uploader.destroy(`stations/${publicId}`);
            }
            const imageUpload = await new Promise((resolve, reject) => {
                const stream = cloudinary_1.default.uploader.upload_stream({ resource_type: "image", folder: "stations" }, (error, result) => {
                    if (error)
